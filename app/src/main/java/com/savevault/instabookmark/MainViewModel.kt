...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun addTags(postId: Long, newTags: List<String>) {
        viewModelScope.launch {
            val post = postDao.getAllPosts().firstOrNull()?.find { it.id == postId } ?: return@launch
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun addTags(postId: Long, newTags: List<String>) {
        viewModelScope.launch {
            val currentList = postDao.getAllPosts().value
    ...
    fun addTags(postId: Long, newTags: List<String>) {
        viewModelScope.launch {
            val currentList = postDao.getAllPosts().value
            val post = currentList.find { it.id == postId } ?: return@launch
            val updated = post.copy(tagsJson = com.savevault.instabookmark.data.TagsConverter.toJson(newTags))
            postDao.update(updated)
        }
    }
}
...
class MainViewModel(private val db: AppDatabase) : ViewModel() {
    ...
    fun addTags(postId: Long, newTags: List<String>) {
        viewModelScope.launch {
            val post = postDao.getAllPosts().value.find { it.id == postId } ?: return@launch
            val updated = post.copy(tagsJson = com.savevault.instabookmark.data.TagsConverter.toJson(newTags))
            postDao.update(updated)
        }
    }
